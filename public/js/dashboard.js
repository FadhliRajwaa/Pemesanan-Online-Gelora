...
const API_URL = 'https://pemesanan-online-gelora.vercel.app/api';
...
async function editRow(collection, id) {
  ...
  for (let field in fields) {
    inputsHTML += `
      <td class="px-6 py-4">
        <input type="text" class="text-black input-field border rounded px-2 py-1" id="${field}-input" value="${fields[field]}" placeholder="Enter ${field}">
      </td>
    `;
  }
